...
class GUI:
    ...
    def add_new_plot_tab(self, parent_class, parent_name, tab_name, init=False):
        ...
        if init:
            pass # print("init fill tabs, skipping creating new tab")
...
class WebSQGroup:
    def __init__(self, ):
        ...
        self.url = '130.237.35.62'   # 12 channels
        self.base_url = f'ws://{self.url}'
        self.number_of_detectors = 12
    ...
    def get_counts(self, num=1):
        loop = asyncio.get_event_loop()
        loop.run_until_complete(websocket_client(self.base_url, self.fetch_counts, n=num))

    # TODO FIXME
    def fetch_counts(self, payload):
...
class NewScanGroup:
    ...
    def choose_param_configs_widget(self, tab):
        ...
        self.params['nr_pixels']['var'].set(12)
        det_no_parts = [ttk.Label(frm['detect'], text="Nr. of pixels"),
                        ...
                        ttk.Radiobutton(frm['detect'], text="12", value=12, variable=self.params['nr_pixels']['var'], command=update_ch),
                        ttk.Radiobutton(frm['detect'], text="24", value=24, variable=self.params['nr_pixels']['var'], command=update_ch, state = 'DISABLED'),
                        ]
        det_no_parts[1].configure(state=tk.DISABLED)
        det_no_parts[3].configure(state=tk.DISABLED)
        # -- Channels:
        ch_parts = [
        ...
        gui.add_to_grid(widg=scanime_parts, rows=[2, 3, 3], cols=[0, 0, 1], sticky=["", "", ""])
