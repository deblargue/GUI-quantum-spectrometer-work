...
def RMSE(a, b, des, cal):
    ...
    # TEMPORARY GRAPH
    plt.figure()
    plt.title("Temporary graph")
    plt.scatter(x_desired, y_prediction)
    plt.plot(x_desired, y_prediction, label="predicted fit")
    plt.scatter(x_desired, y_calibration, label="calibrated points")
...
def connect_serial(port='COM4'):
    ...
    if handle:
        print("Successfully connected to PORT:", 'COM4', ", Serial handle:", handle)
        time.sleep(2)
        return handle
    else:
...
def send_cmd(type, handle, value=None):
    ...
    if time_iter == -1:
        time_iter = calcualte_time_iter(handle, value)
    ...
    if not demo:
        ...
        # 3) WAIT FOR OK SIGN
        res = ''
        off_time = 0.5  # seconds between each request
...
def calcualte_time_iter(handle, new_val):
    ...
    return np.max([int(60 * (np.abs(float(current_nm) - float(new_val)) / float(scan_speed))), max_wait])  # in seconds
...
def get_menu(step_size):
    menu_str = f"\n------------------------------MENU--------------------------------" \
               ...
               f"\n  ->     c / C / change   (change to a new calibration wavelength)" \
...
def main():
    ...
    #print(f" START DEVICE NM: {current_nm} nm")
    ...
    while running:   # This is the program loop that will keep asking for new wave lengths until we are done
        ...
        if res in ['u', 'U', 'up']:  # toggle up
            ...
            # NOTE: UNSURE IF WE DO THIS HERE
            saved_calibrations[current_desired_nm] = current_nm
        ...
        if new_nm is not None:
            if 0 <= new_nm <= 1000:  # if in range, send value
                # 7) Send new value
                if realtest:
                    print(f"should send value: {new_nm}")
    ...
    # TODO: finish plot
    # plot_calibration(x_list, y_list)
